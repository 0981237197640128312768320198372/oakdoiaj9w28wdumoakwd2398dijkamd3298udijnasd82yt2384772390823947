  ...
  try {
    ...
    const userInfo =
      (await retryOperation(
        () => getGoogleSheetsData(process.env.___SPREADSHEET_ID as string, userInfoSheetRange),
        3, // Retries
        5000 // Delay in ms
      )) || [];
    ...
    for (const { name, quantity, duration, price } of selectedProducts) {
      ...
      for (let i = 0; i < quantity; i++) {
        ...
        const orderDateFormatter = new Intl.DateTimeFormat('en-GB', orderDateOptions);
        const expireDateFormatter = new Intl.DateTimeFormat('en-GB', expireDateOptions);
