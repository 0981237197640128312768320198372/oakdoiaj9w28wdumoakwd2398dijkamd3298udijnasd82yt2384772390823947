...
const rangeModels: Record<string, { appName: string; accessType: string; columns: string[] }> = {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    for (const [range, model] of Object.entries(rangeModels)) {
      const sheetData =
        (await getGoogleSheetsData(process.env.___SPREADSHEET_ID as string, range)) || [];
