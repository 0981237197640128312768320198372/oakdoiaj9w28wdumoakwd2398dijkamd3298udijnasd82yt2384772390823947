...
const MobileNav = () => {
  ...
  return (
    <Drawer>
      <DrawerTrigger className='xl:hidden dark:hover:bg-white/10 hover:bg-black/5 p-2 focus:outline-none focus:ring-0 dark:focus:bg-dark-700'>
        <CiMenuBurger className='duration-500 text-dark-800 dark:text-primary text-3xl' />
      </DrawerTrigger>
      <DrawerContent className='bg-light-200 dark:bg-dark-600 border-none px-5 gap-10 focus:outline-none focus:ring-0'>
        <div className='grid grid-cols-2 w-full h-fit gap-1'>
          {navButtons.map((nav, i) => (
            <Link
              href={nav.url}
              key={i}
              className={`dark:text-light-400 w-full flex justify-start items-start h-12 md:text-sm font-medium group duration-200 p-2 ${
                path === nav.url
