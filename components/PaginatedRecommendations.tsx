...
const fetchTotalItems = async () => {
  const res = await fetch(
    `/api/CRUDsheet/read?sheet=MovieAndSeriesRecommendation&range=A2:E`,
    {
      headers: {
...
const fetchRecommendations = async (page: number, limit: number) => {
  ...
  const res = await fetch(
    `/api/CRUDsheet/read?sheet=MovieAndSeriesRecommendation&range=A2:E&limit=${limit}&offset=${offset}`,
    {
      headers: {
        "x-api-key": "1092461893164193047348723920781631",
      },
