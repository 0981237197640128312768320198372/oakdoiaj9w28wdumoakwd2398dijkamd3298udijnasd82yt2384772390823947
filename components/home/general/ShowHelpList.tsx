...
import { Timeline } from '@/components/home/general/Timeline';
  ...
  const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const filtered = helps.filter((help) => {
      ...
      return (
        ...
        steps.some(
          (step) =>
            step.step.toLowerCase().includes(term) || step.description.toLowerCase().includes(term)
        )
      );
    });
        ...
        <motion.div
          ...
          <Timeline
            data={selectedHelp.steps.map((step) => ({
              ...
              content: (
                <div>
                  <p className="text-light-600 text-xs md:text-sm mb-8">{step.description}</p>
