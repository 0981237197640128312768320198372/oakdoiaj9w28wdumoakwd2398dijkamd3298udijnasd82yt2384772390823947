...
export class LineService {
  ...
  static generateVerificationCode(): LineVerificationCode {
    const code = 'DS-' + crypto.randomInt(100000, 999999).toString();
  ...
  static extractVerificationCode(text: string): string | null {
    ...
    // Match DS- followed by 6 digits (case insensitive)
    const match = text.match(/DS-\d{6}/i);
